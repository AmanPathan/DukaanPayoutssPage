...
const Orders = () => {
    ...
    const handleSort = () => {
        setFlag(true);
        tableData.sort((a, b) => a.orderId - b.orderId);
        setTable(tableData);
    }
    return (
        <>
            <div className="bg-white border border-white  rounded-lg p-3 space-y-3 max-sm:p-0 max-sm:py-2 max-sm:w-screen" style={{ boxShadow: "0px 2px 6px 0px rgba(26, 24, 30, 0.04)" }}>
                ...
                <section className='flex justify-between max-sm:items-center max-sm:px-2'>
                    ...
                    <aside className="flex items-center space-x-4 max-sm:text-xs max-sm:space-x-2">
                        <span onClick={handleSort} className="flex items-center space-x-3 bg-white px-3 py-1.5 border rounded border-border shadow-sm max-sm:space-x-1 max-sm:px-1.5 hover:cursor-pointer">
                ...
                <section className='flex-row justify-center w-full text-left max-sm:table-fixed max-sm:w-full'>
                    <div className='max-sm:w-screen'>
                        <div className="bg-[#F2F2F2] flex flex-row items-center justify-between">
                            <div className='w-1/5 table_title text-left text-text py-2.5 px-3 font-medium bg-[#F2F2F2] rounded-l text-sm max-sm:text-[8.5px] max-sm:px-0 max-sm:pl-1 max-sm:py-1.5 max-sm:w-10'>Order ID</div>
                            <div className='w-1/5 table_title text-left text-text py-2.5 px-3 font-medium bg-[#F2F2F2] text-sm max-sm:text-[8.5px] max-sm:px-0 max-sm:py-1.5 max-sm:w-11'>Status</div>
                            <div className='w-1/5 table_title text-left text-text py-2.5 px-3 font-medium bg-[#F2F2F2] text-sm max-sm:text-[8.5px] max-sm:px-0 max-sm:py-1.5 max-sm:w-14'>Transaction ID</div>
                            <div className='w-1/5 table_title text-left text-text py-2.5 px-3 font-medium bg-[#F2F2F2] text-sm max-sm:text-[8.5px] max-sm:px-0 max-sm:py-1.5 max-sm:w-16'>Refund date</div>
                            <div className="w-1/5 table_title text-right text-text py-2.5 px-3 font-medium bg-[#F2F2F2] rounded-r text-sm max-sm:text-[8.5px] max-sm:px-0 max-sm:pr-1 max-sm:py-1.5 max-sm:w-14">Order amount</div>
                        </div>
                    </div>
                    <div className="max-sm:w-screen">
                        {!flag ?
                            tableData.map(({ orderId, successful, transactionID, refundDate, orderAmount }, index) => (
                                <TableRow key={orderId} orderId={orderId} successful={successful} transactionID={transactionID} refundDate={refundDate} orderAmount={orderAmount} index={index} />
                            ))
                            :
                            table.map(({ orderId, successful, transactionID, refundDate, orderAmount }, index) => (
                                <TableRow key={orderId} orderId={orderId} successful={successful} transactionID={transactionID} refundDate={refundDate} orderAmount={orderAmount} index={index} />
                            ))
                        }
                    </div>
