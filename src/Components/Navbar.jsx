...
const Navbar = ({showSidebar,setSidebar}) => {
  return (
    <nav className={`bg-white flex w-full items-center px-8 py-3 space-x-4 sticky top-0 z-30 ${showSidebar?"max-sm:bg-gray-500/70 max-sm:pt-6 max-sm:pb-4":""} max-sm:px-0 max-sm:space-x-2 max-sm:justify-between`}>
       {showSidebar
       ?<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className=" text-white rounded-full ml-5 p-1 bg-red-500 w-7 h-7 shrink-0 sm:hidden" onClick={()=>setSidebar(false)}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" />
         </svg>
      
       :<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 shrink-0 sm:hidden" onClick={()=>setSidebar(true)}>
